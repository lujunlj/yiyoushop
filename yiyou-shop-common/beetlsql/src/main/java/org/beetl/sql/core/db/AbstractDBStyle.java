...
import org.beetl.sql.core.annotatoin.builder.AttributeInsertBuilder;
    ...
    public SQLSource genDeleteById(Class<?> cls) {
        ...
        if(classDesc.getLogicDeleteAttrName()==null) {
            ...
            return new SQLTableSource(new StringBuilder("update  ")
                    ...
                    .append(col).append(" = ").append(this.logicDeleteValue == 1?classDesc.getLogicDeleteAttrValue():this.logicDeleteValue).append(condition).toString());
        }
...
//            return baseAttributeBuilder.toSql(this,fieldName,colName,handler.getBeanAnnotaton(),table);
    ...
    protected String appendInsertValue(Class<?> c, TableDesc table, String fieldName,String colName) {
        ...
        if(handler!=null&&handler.supportPersistGen()){
        	AttributePersistBuilder baseAttributeBuilder = (AttributePersistBuilder)handler.getInstance();
            sql = baseAttributeBuilder.toSql(this,fieldName,colName,handler.getBeanAnnotaton(),table);
        }else if(handler!= null && handler.supportInsertGen()){
            AttributeInsertBuilder attributeInsertBuilder = (AttributeInsertBuilder)handler.getInstance();
            sql = attributeInsertBuilder.toSql(this,fieldName,colName,handler.getBeanAnnotaton(),table);
        }
